...
{
	readonly ILogger<SafeConnectionFactory> _logger;
	readonly Security.ISecretManager _secretManager;
	...
	DateTime _lastHealthCheck = DateTime.MinValue;
	bool _isHealthy = false;
	bool _disposed = false;

	readonly TimeSpan _connectionTimeout = TimeSpan.FromSeconds (30);
	readonly TimeSpan _healthCheckInterval = TimeSpan.FromMinutes (5);
	...
	{
		ObjectDisposedException.ThrowIf (_disposed, nameof (SafeConnectionFactory));
	}
