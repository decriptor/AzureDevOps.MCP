...
{
	readonly IAzureDevOpsService _innerService;
	readonly ICacheService _cache;
	readonly IPerformanceService _performance;
	readonly ILogger<CachedAzureDevOpsService> _logger;
	...
	{
		var transaction = SentrySdk.StartTransaction ("GetProjects", "azure_devops.read");
		...
		try {
			var result = await _cache.GetOrSetAsync ("projects", async () => {
				var span = transaction?.StartChild ("azure_devops.api_call", "GetProjects");
				var sw = Stopwatch.StartNew ();
				try {
					span?.SetTag ("cache.hit", "false");
					var innerResult = await _innerService.GetProjectsAsync ();
					_performance.RecordApiCall ("GetProjects", sw.ElapsedMilliseconds, true);

					span?.SetTag ("projects.count", innerResult.Count ().ToString ());
					span?.SetTag ("duration.ms", sw.ElapsedMilliseconds.ToString ());
					transaction?.SetExtra ("projects.count", innerResult.Count ());

					return innerResult.ToList ();
				} catch (Exception ex) {
	...
	public async Task<WorkItem?> GetWorkItemAsync (int id)
	{
		...
		var cached = await _cache.GetAsync<WorkItem> (cacheKey);
		if (cached != null) {
			return cached;
		}
