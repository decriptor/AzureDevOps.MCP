...
public sealed class ModernSensitiveDataFilter : ISensitiveDataFilter
{
    ...
    private static readonly FrozenDictionary<string, Regex> SensitivePatterns = new Dictionary<string, Regex>
    {
        ...
        ["password"] = new Regex(@"(password|pwd|pass|secret|key)[\""\s]*[:=][\""\s]*[^\s\"""",;]+", RegexOptions.Compiled | RegexOptions.IgnoreCase),
    ...
    private const string RedactionReplacement = "[REDACTED]";
    ...
    private ExceptionInfo FilterException(ExceptionInfo exception)
    {
