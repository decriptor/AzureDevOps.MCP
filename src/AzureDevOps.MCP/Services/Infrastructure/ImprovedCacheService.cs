...
public class ImprovedCacheService : ICacheService, IDisposable
{
	...
	readonly ICacheStrategy _strategy;
	readonly IMemoryPressureManager _memoryPressureManager;
	readonly ILogger<ImprovedCacheService> _logger;
	readonly ConcurrentDictionary<string, DateTime> _keyAccessTimes = new ();
	readonly Timer _cleanupTimer;
	readonly object _cleanupLock = new ();
	bool _disposed;
	...
	public AzureDevOps.MCP.Services.CacheStatistics GetStatistics ()
	{
		ThrowIfDisposed ();

		return new AzureDevOps.MCP.Services.CacheStatistics {
			EntryCount = _keyAccessTimes.Count,
			HitRate = 0.0, // Simple implementation - would need actual tracking
			TotalSizeBytes = 0, // Simple implementation - would need actual calculation
			HitCount = 0,
			MissCount = 0
		};
	}

	Task EvictLeastRecentlyUsedAsync ()
	{
		if (_disposed) {
			return;
		}
	...
	void OnMemoryPressureChanged (object? sender, MemoryPressureEventArgs e)
	{
		if (e.IsHighPressure) {
