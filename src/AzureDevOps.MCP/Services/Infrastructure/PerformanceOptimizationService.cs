...
{
	readonly ILogger<PerformanceOptimizationService> _logger;
	readonly PerformanceConfiguration _config;

	// Use frozen collections for read-only performance-critical data
	static readonly FrozenDictionary<string, TimeSpan> DefaultCacheDurations = new Dictionary<string, TimeSpan> {
	...
	static readonly FrozenSet<string> HighFrequencyOperations = new HashSet<string>
	{
		"GetWorkItems",
		"GetBuilds",
		"GetTestResults",
	...
	readonly ConcurrentDictionary<string, PerformanceMetrics> _operationMetrics = new ();
	readonly ConcurrentQueue<PerformanceEvent> _recentEvents = new ();
	readonly Timer _metricsCleanupTimer;
	...
	static bool IsUserSpecificOperation ()
	{
	...
	static TimeSpan AdjustDurationBasedOnMetrics (TimeSpan baseDuration, PerformanceMetrics metrics)
	{
	...
	void RecordSuccessfulOperation (string operationName, TimeSpan duration, long memoryDelta)
	{
	...
	void RecordFailedOperation (string operationName, TimeSpan duration, Exception exception)
	{
		_operationMetrics.AddOrUpdate (operationName,
			new PerformanceMetrics (
				OperationName: operationName,
	...
	void RecordAllocation (string typeName, long estimatedSize)
	{
	...
	static long EstimateObjectSize<T> (T obj)
	{
	...
	static string GetOptimizationSuggestion (string operationName, PerformanceMetrics metrics)
	{
	...
	static WarmingPriority CalculateWarmingPriority (PerformanceMetrics metrics)
	{
	...
	static double CalculateEstimatedBenefit (PerformanceMetrics metrics)
	{
	...
	static TimeSpan CalculatePreloadTime (PerformanceMetrics metrics)
	{
	...
	void CleanupOldMetrics (object? state)
	{
		try {
