...
public sealed class ModernRateLimitingMiddleware
{
	...
	readonly ILogger<ModernRateLimitingMiddleware> _logger;
	readonly RateLimitingConfiguration _config;
	readonly IRateLimitStore _store;

	// Use frozen dictionary for rate limit configurations
	static readonly FrozenDictionary<string, RateLimitRule> DefaultRules = new Dictionary<string, RateLimitRule> {
	...
	static readonly FrozenSet<string> ExemptPaths = new HashSet<string>
	{
		"/health",
		"/health/live",
		"/health/ready",
	...
	static string GetCombinedClientId (HttpContext context)
	{
	...
	static string HashClientId (string clientId)
	{
	...
	async Task<RateLimitResult> CheckRateLimitAsync (string clientId, CancellationToken cancellationToken)
	{
	...
	int GetLimitForRule (string ruleName) => ruleName switch {
	...
	static TimeSpan CalculateRetryAfter (DateTimeOffset resetTime)
	{
		var retryAfter = resetTime - DateTimeOffset.UtcNow;
		return retryAfter > TimeSpan.Zero ? retryAfter : TimeSpan.FromSeconds (1);
	}

	static void AddRateLimitHeaders (HttpContext context, RateLimitResult result)
	{
	...
	async Task HandleRateLimitExceeded (HttpContext context, RateLimitResult result)
	{
