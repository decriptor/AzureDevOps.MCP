...
public static class TracingHelper
{
		...
		bool isTransaction = false)
	{
		...
		var sentryOperation = isTransaction
	...
	{
		var transaction = SentrySdk.StartTransaction (operationName, GetOperationCategory (operationName));
		using var perfTracker = performance.TrackOperation (operationName, metadata);
		...
		var result = await cache.GetOrSetAsync (cacheKey, async () => {
			var span = transaction?.StartChild ("azure_devops.api_call", operationName);
			var sw = Stopwatch.StartNew ();

			try {
				span?.SetTag ("cache.hit", "false");
				var resultFromOperation = await operation ();

				// Add result metadata
				if (resultFromOperation is IEnumerable<object> enumerable) {
	...
	public static async Task<T> TraceWriteOperation<T> (
		...
		Dictionary<string, object>? metadata = null)
	{
		var transaction = SentrySdk.StartTransaction (operationName, "azure_devops.write");
		using var perfTracker = performance.TrackOperation (operationName, metadata);
	...
	static string GetOperationCategory (string operationName)
	{
	...
	static string GetOperationType (string operationName)
	{
		return operationName switch {
	...
	static string ExtractOrgName (string organizationUrl)
	{
		try {
