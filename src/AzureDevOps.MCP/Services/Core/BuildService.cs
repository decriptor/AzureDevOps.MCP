...
public class BuildService : IBuildService
{
	...
	public async Task<IEnumerable<BuildDefinitionReference>> GetBuildDefinitionsAsync(string projectNameOrId, int limit = 100, CancellationToken cancellationToken = default)
	{
		...
		return await _errorHandler.ExecuteWithErrorHandlingAsync(async (ct) =>
		{
			...
			// For now, return simulated data as the specific build packages are not available
	...
	private static IEnumerable<BuildDefinitionReference> GenerateSimulatedBuildDefinitions(string projectName, int limit)
	{
	...
	private static BuildDefinition? GenerateSimulatedBuildDefinition(string projectName, int definitionId)
	{
		if (definitionId > 10) return null; // Simulate not found
	...
	private static IEnumerable<Build> GenerateSimulatedBuilds(string projectName, int? definitionId, int limit)
	{
		var random = new Random();
	...
	private static Build? GenerateSimulatedBuild(string projectName, int buildId)
	{
		if (buildId > 100) return null; // Simulate not found
	...
	private static IEnumerable<TaskAgentPool> GenerateSimulatedAgentPools()
	{
	...
	private static IEnumerable<TaskAgent> GenerateSimulatedAgents(int poolId)
	{
		if (poolId == 1) yield break; // Hosted pool has no visible agents
