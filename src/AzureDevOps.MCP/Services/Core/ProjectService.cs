...
public class ProjectService : IProjectService
{
	...
	public async Task<IEnumerable<TeamProjectReference>> GetProjectsAsync (CancellationToken cancellationToken = default)
	{
		...
		return await _errorHandler.ExecuteWithErrorHandlingAsync (async ct => {
			// Check cache first
			var cached = await _cacheService.GetAsync<List<TeamProjectReference>> (CacheKeyPrefix);
			if (cached != null) {
			...
			// Cache the results
			await _cacheService.SetAsync (CacheKeyPrefix, projectList, DefaultCacheExpiration);
	...
	public async Task<TeamProject?> GetProjectAsync (string projectNameOrId, CancellationToken cancellationToken = default)
	{
		...
		return await _errorHandler.ExecuteWithErrorHandlingAsync (async ct => {
			...
			// Check cache first
			var cached = await _cacheService.GetAsync<TeamProject> (cacheKey);
			if (cached != null) {
			...
			try {
				...
				if (project != null) {
					// Cache the result
					await _cacheService.SetAsync (cacheKey, project, DefaultCacheExpiration);
	...
	public async Task<IEnumerable<ProjectProperty>> GetProjectPropertiesAsync (string projectNameOrId, CancellationToken cancellationToken = default)
	{
		...
		return await _errorHandler.ExecuteWithErrorHandlingAsync (async ct => {
			...
			// Check cache first
			var cached = await _cacheService.GetAsync<List<ProjectProperty>> (cacheKey);
			if (cached != null) {
