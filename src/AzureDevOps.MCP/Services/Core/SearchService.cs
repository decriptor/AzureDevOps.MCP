...
public class SearchService : ISearchService
{
	...
	static IEnumerable<CodeSearchResult> GenerateSimulatedCodeSearchResults (string searchText, string? projectName, string? repositoryName, int limit)
	{
	...
	static List<CodeMatch> GenerateCodeMatches (string searchText, int matchCount)
	{
		var random = new Random (searchText.GetHashCode ());
	...
	static IEnumerable<WorkItemSearchResult> GenerateSimulatedWorkItemSearchResults (string searchText, string? projectName, string? workItemType, int limit)
	{
		var random = new Random (searchText.GetHashCode ());
	...
	static List<string> GenerateTags (string searchText, Random random)
	{
	...
	static List<WorkItemMatch> GenerateWorkItemMatches (string searchText, int matchCount)
	{
		var random = new Random (searchText.GetHashCode ());
	...
	static IEnumerable<FileSearchResult> GenerateSimulatedFileSearchResults (string fileName, string? projectName, string? repositoryName, int limit)
	{
	...
	static IEnumerable<PackageSearchResult> GenerateSimulatedPackageSearchResults (string packageName, string? feedName, int limit)
	{
		...
		for (int i = 1; i <= Math.Min (limit, 10); i++) {
			...
			yield return new PackageSearchResult {
				...
				Authors = [authors[random.Next (authors.Length)]],
				Tags = [packageName.ToLower (), packageType.ToLower (), "library", "framework"],
