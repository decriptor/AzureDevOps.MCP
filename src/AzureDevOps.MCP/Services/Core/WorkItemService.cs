...
public class WorkItemService : IWorkItemService
{
	...
	readonly ILogger<WorkItemService> _logger;

	const string CacheKeyPrefix = "workitems";
	static readonly TimeSpan DefaultCacheExpiration = TimeSpan.FromMinutes (2);
		...
		string projectName, int limit = 100, CancellationToken cancellationToken = default)
	{
		...
		return await _errorHandler.ExecuteWithErrorHandlingAsync (async ct => {
			...
			// Check cache first
			var cached = await _cacheService.GetAsync<List<WorkItem>> (cacheKey);
			if (cached != null) {
			...
			var workItems = await QueryWorkItemsInternalAsync (projectName, wiql, limit, cancellationToken);
			var workItemsList = workItems.ToList ();

			// Cache the results
			await _cacheService.SetAsync (cacheKey, workItemsList, DefaultCacheExpiration);
		...
		int id, CancellationToken cancellationToken = default)
	{
		...
		return await _errorHandler.ExecuteWithErrorHandlingAsync (async ct => {
			...
			// Check cache first
			var cached = await _cacheService.GetAsync<WorkItem> (cacheKey);
			if (cached != null) {
			...
			try {
				...
				if (workItem != null) {
					// Cache the result
					await _cacheService.SetAsync (cacheKey, workItem, DefaultCacheExpiration);
		...
		int id, CancellationToken cancellationToken = default)
	{
		...
		return await _errorHandler.ExecuteWithErrorHandlingAsync (async ct => {
			...
			await _cacheService.SetAsync (cacheKey, revisionsList, TimeSpan.FromMinutes (10));
	...
	async Task<IEnumerable<WorkItem>> QueryWorkItemsInternalAsync (
