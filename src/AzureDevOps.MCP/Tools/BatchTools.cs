...
public class BatchTools
{
    ...
    private readonly ILogger<BatchTools> _logger;
        ...
        [Description("Array of work item IDs to retrieve")] int[] workItemIds)
    {
        ...
        try
        {
            ...
            return new
            {
                ...
                {
                    id = r.workItem!.Id,
                    title = r.workItem.Fields.ContainsKey("System.Title") ? r.workItem.Fields["System.Title"]?.ToString() : "No title",
                    state = r.workItem.Fields.ContainsKey("System.State") ? r.workItem.Fields["System.State"]?.ToString() : "Unknown",
                    type = r.workItem.Fields.ContainsKey("System.WorkItemType") ? r.workItem.Fields["System.WorkItemType"]?.ToString() : "Unknown",
                    url = r.workItem.Url
                }),
    ...
    private async Task<(int id, Microsoft.TeamFoundation.WorkItemTracking.WebApi.Models.WorkItem? workItem, string? error)> GetWorkItemWithErrorHandling(int id)
    {
        try
        {
    ...
    private async Task<(string path, string? content, string? error)> GetFileContentWithErrorHandling(string projectName, string repositoryId, string path)
    {
        try
        {
    ...
    private async Task<(string path, IEnumerable<Microsoft.TeamFoundation.SourceControl.WebApi.GitItem>? items, string? error)> ListItemsWithErrorHandling(string projectName, string repositoryId, string path)
    {
        try
        {
