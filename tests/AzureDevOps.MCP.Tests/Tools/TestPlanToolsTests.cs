...
{
	Mock<ITestService> _mockTestService = null!;
	Mock<IPerformanceService> _mockPerformanceService = null!;
	Mock<ILogger<TestPlanTools>> _mockLogger = null!;
	Mock<IDisposable> _mockTracker = null!;
	TestPlanTools _testPlanTools = null!;

	static JsonElement GetJsonElement (object obj)
	{
	...
	public async Task GetTestPlansAsync_ShouldReturnTestPlans_WhenServiceReturnsData ()
	{
		...
		var resultData = GetJsonElement (result);
		resultData.GetProperty ("success").GetBoolean ().Should ().BeTrue ();
		resultData.GetProperty ("projectName").GetString ().Should ().Be (projectName);
		resultData.GetProperty ("totalCount").GetInt32 ().Should ().Be (2);
	...
	public async Task GetTestPlanAsync_ShouldReturnTestPlan_WhenPlanExists ()
	{
		...
		resultData.GetProperty ("success").GetBoolean ().Should ().BeTrue ();
		resultData.GetProperty ("projectName").GetString ().Should ().Be (projectName);
		resultData.GetProperty ("testPlan").GetProperty ("id").GetInt32 ().Should ().Be (planId);
		resultData.GetProperty ("testPlan").GetProperty ("name").GetString ().Should ().Be ("Specific Test Plan");
	}
	...
	public async Task GetTestPlanAsync_ShouldReturnNotFound_WhenPlanDoesNotExist ()
	{
		...
		result.Should ().NotBeNull ();
		var resultData = GetJsonElement (result);
		resultData.GetProperty ("success").GetBoolean ().Should ().BeFalse ();
		resultData.GetProperty ("message").GetString ().Should ().Contain ($"Test plan {planId} not found");
	}
	...
	public async Task GetTestSuitesAsync_ShouldReturnTestSuites_WhenServiceReturnsData ()
	{
		...
		resultData.GetProperty ("success").GetBoolean ().Should ().BeTrue ();
		resultData.GetProperty ("projectName").GetString ().Should ().Be (projectName);
		resultData.GetProperty ("planId").GetInt32 ().Should ().Be (planId);
		resultData.GetProperty ("totalCount").GetInt32 ().Should ().Be (2);
	}
	...
	public async Task GetTestRunsAsync_ShouldReturnTestRuns_WhenServiceReturnsData ()
	{
		...
		resultData.GetProperty ("success").GetBoolean ().Should ().BeTrue ();
		resultData.GetProperty ("projectName").GetString ().Should ().Be (projectName);
		resultData.GetProperty ("totalCount").GetInt32 ().Should ().Be (1);
	}
	...
	public async Task GetTestRunAsync_ShouldReturnTestRun_WhenRunExists ()
	{
		...
		resultData.GetProperty ("success").GetBoolean ().Should ().BeTrue ();
		resultData.GetProperty ("projectName").GetString ().Should ().Be (projectName);
		resultData.GetProperty ("testRun").GetProperty ("id").GetInt32 ().Should ().Be (runId);
		resultData.GetProperty ("testRun").GetProperty ("name").GetString ().Should ().Be ("Specific Test Run");
	}
	...
	public async Task GetTestRunAsync_ShouldReturnNotFound_WhenRunDoesNotExist ()
	{
		...
		// Assert
		result.Should ().NotBeNull (); var resultData = GetJsonElement (result);
		resultData.GetProperty ("success").GetBoolean ().Should ().BeFalse ();
		resultData.GetProperty ("message").GetString ().Should ().Contain ($"Test run {runId} not found");
	}
	...
	public async Task GetTestResultsAsync_ShouldReturnTestResults_WhenServiceReturnsData ()
	{
		...
		_mockTestService
			...
			.ReturnsAsync (testResults);        // Act
		var result = await _testPlanTools.GetTestResultsAsync (projectName, runId);

		// Assert
		result.Should ().NotBeNull ();

		// Use reflection instead of dynamic to access properties
		var resultType = result.GetType ();
		var successProperty = resultType.GetProperty ("success");
		var projectNameProperty = resultType.GetProperty ("projectName");
		var runIdProperty = resultType.GetProperty ("runId");
		var totalCountProperty = resultType.GetProperty ("totalCount");

		((bool)successProperty!.GetValue (result)!).Should ().BeTrue ();
		((string)projectNameProperty!.GetValue (result)!).Should ().Be (projectName);
		((int)runIdProperty!.GetValue (result)!).Should ().Be (runId);
		((int)totalCountProperty!.GetValue (result)!).Should ().Be (2);
	}
	...
	public async Task GetTestPlansAsync_ShouldThrowInvalidOperationException_WhenServiceThrows ()
	{
		...
		// Act & Assert
		var thrownException = await Assert.ThrowsExactlyAsync<InvalidOperationException> (
